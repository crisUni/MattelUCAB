...
export function Sidebar({
  ...
  const { rolActual, isBackOffice } = useSession();
    ...
    >
      ...
      <div className="border-t border-slate-100 p-3">
        <div className={`flex items-center gap-2 rounded-xl bg-slate-50 p-2.5 ${collapsed ? "justify-center" : ""}`}>
          <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg text-xs font-bold text-white ${isBackOffice ? "bg-navy-600" : "bg-brand-500"}`}>
          ...
          {!collapsed && (
            <div className="animate-fade-in-fast min-w-0 leading-tight">
              ...
              <p className="text-[10px] text-slate-400">{isBackOffice ? "Back-Office" : "Front-Office"}</p>
            </div>
          )}
        </div>
      </div>
