...
const CAPS = [
  ...
  { permiso: "perm-costos-ver", label: "Ver costos y márgenes", modulo: "Costos" },
...
export function SimuladorTab() {
  const { roles, rolActual, setRolActualId, can, puedeVerCostos } = useSession();
  ...
  return (
    <div className="animate-fade-in">
      <SectionHeader
        ...
        subtitle="Inicia sesión como cualquier rol y observa en vivo cómo se filtran menús, columnas de costo y acciones. El cambio se refleja en todo el sistema."
      />
      ...
      <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-navy-500">Roles</p>
            <div className="space-y-1.5">
              {roles.map((r) => <RolPick key={r.id} r={r} active={r.id === rolActual.id} onClick={() => setRolActualId(r.id)} />)}
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-4">
          <Card className="relative overflow-hidden bg-gradient-to-br from-brand-500 to-grape-600 p-5 text-white">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <Stat label="Costos de producción" value={puedeVerCostos ? "Visibles" : "Ocultos"} />
            </div>
          </Card>
          ...
          <Card className="p-5">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {CAPS.map((c) => {
                ...
                const esCostos = c.modulo === "Costos";
                const bloqueadoCostos = esCostos && !ok;
                return (
                  <div key={c.permiso} className={`flex items-center gap-2.5 rounded-xl border px-3 py-2.5 transition ${ok ? "border-emerald-200 bg-emerald-50/60" : "border-slate-200 bg-slate-50"}`}>
                    <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg ${ok ? "bg-emerald-500 text-white" : bloqueadoCostos ? "bg-rose-100 text-rose-500" : "bg-slate-200 text-slate-400"}`}>
                      {ok ? <IconCheck className="h-4 w-4" /> : bloqueadoCostos ? <IconLock className="h-3.5 w-3.5" /> : <IconClose className="h-3.5 w-3.5" />}
                    </span>
                    <div className="min-w-0 leading-tight">
                      <p className={`truncate text-sm font-medium ${ok ? "text-navy-700" : "text-slate-400"}`}>{c.label}</p>
                      <p className="text-[11px] text-slate-400">{c.modulo}{esCostos && " · reservado a back-office"}</p>
                    </div>
                  </div>
                );
              })}
...
function RolPick({ r, active, onClick }: { r: { nombre: string; permisosIds: string[] }; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left transition ${active ? "bg-brand-50 ring-1 ring-brand-300" : "hover:bg-slate-50"}`}>
      <span className={`h-2 w-2 shrink-0 rounded-full ${active ? "bg-brand-500" : "bg-slate-300"}`} />
      <span className="flex min-w-0 flex-1 items-center justify-between gap-2">
        <span className="truncate text-sm font-semibold text-navy-700">{r.nombre}</span>
