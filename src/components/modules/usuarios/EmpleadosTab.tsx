...
import {
  getEmpleados, crearEmpleado, getDepartamentosOpc, getCargosOpc,
  type Empleado, type Opcion,
} from "../../../services/api";
...
export function EmpleadosTab() {
  ...
  const columns: Column<Empleado>[] = [
    ...
    { key: "direccion", header: "Dirección", cell: (e) => <span className="text-xs text-slate-400">{e.direccion}</span> },
  ];
  ...
  return (
    <div className="animate-fade-in">
      ...
      {creando && <NuevoEmpleadoForm onCancel={() => setCreando(false)} onSaved={async () => { setCreando(false); await recargar(); }} />}
    </div>
  );
}

function NuevoEmpleadoForm({ onCancel, onSaved }: { onCancel: () => void; onSaved: () => void }) {
  ...
  async function submit() {
    ...
    setGuardando(true);
    try {
      await crearEmpleado({ pnombre: pnombre.trim(), snombre: snombre.trim(), papellido: papellido.trim(), sapellido: sapellido.trim(), direccion: direccion.trim(), dep, car });
      onSaved();
    } catch (e) {
  ...
  return (
    <Modal open onClose={onCancel} size="lg"
      ...
      subtitle="Registra una persona del personal y su adscripción (departamento + cargo)."
